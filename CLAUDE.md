...
- **`.claude/skills/`** — `plenya-deck`, `linkedin-week`, `lecture-builder` (`/aula`), `ebook-builder` (`/ebook`), `plano-paciente` (`/plano` — devolutiva de resultados), `prontuario-prod` (`/prontuario-prod` — levar prontuário para produção), `responder-insta`, `pptx`. Ver [.claude/social/](.claude/social/).

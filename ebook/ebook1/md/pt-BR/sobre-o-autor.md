...
*Esta é a versão publicada no livro (acima, entre os marcadores).*
...
- **Ordem dos títulos**: especialidade → formação → cargos → sociedades → áreas de atuação. Ordem consagrada em bios médicas brasileiras.
- **Tom**: informativo e sóbrio. Evitei adjetivos de autopromoção ("renomado", "referência", "premiado") — o livro todo evita postura de guru, então a bio também.
- **Encerramento conectado ao livro**: as três versões terminam amarrando a carreira ao livro — "é dessa prática que nasce este livro" — o que reforça credibilidade sem inflacionar.
- **Credenciais regulatórias (CRM-PR + RQE)**: incluídas em todas as versões. Em livros médicos brasileiros, o CRM é quase obrigatório por convenção editorial e sinalização de legitimidade.
- **Redes sociais**: Instagram incluído apenas nas versões padrão e estendida. Micro bio fica limpa.
- **Uso do nome completo vs curto**: nome completo na primeira menção de cada versão; depois, "Dr. Amaral Filho" (forma respeitosa em PT) se o texto se repetir.

# Revisão Editorial Completa — Livro ANTES (18 capítulos)

**Data:** 2026-04-21
**Escopo:** leitura linear dos 24 arquivos md em `md/pt-BR/`, com foco em arcos narrativos, estilo, erros, incoerências e redundâncias.

---

## Resumo executivo

O livro está editorialmente sólido. A reestruturação de 14 → 18 capítulos foi executada sem cicatrizes visíveis — os pacientes atravessam as novas fronteiras de capítulo coerentemente, as transições entre caps funcionam, e o tom autoral (1ª pessoa, frase curta, evidência com pedigree, coloquialidade temperada por rigor clínico) se mantém uniforme. Os biomarcadores dos seis personagens batem com o que `personagens.md` documenta. Nenhum erro crítico clínico ou narrativo foi encontrado.

A única inconsistência factual real encontrada foi uma divergência entre `personagens.md` e o Cap 5 sobre a dose da rosuvastatina de Marcos (20 mg × 10 mg); corrigi o `personagens.md` para 10 mg, que é a dose descrita no Cap 5. Fora isso, os achados são editoriais menores ou decisões de julgamento que dependem da preferência do autor — listados abaixo.

Pontos fortes notáveis: (1) o par Ana/Ricardo como âncoras dos dois capítulos do pilar I funciona muito bem — Ana representa trabalho antes do evento, Ricardo reconstrução depois do evento; (2) os bridges entre pilares são explícitos e narrativamente suaves; (3) o Cap 13 (Conexão, Propósito e Sentido) é o capítulo mais literariamente denso do livro e ergue o nível médio.

Pontos de atenção: (1) Marina (esposa de Ricardo) aparece pela primeira vez no Cap 13 sem ter sido apresentada nem mesmo como "a esposa" em Caps 1 ou 4; (2) a palavra "dose baixa" para a rosuvastatina 10 mg é usada em múltiplos capítulos — defensável no contexto narrativo, mas pode soar tecnicamente impreciso para leitor-médico; (3) algumas redundâncias de definição (ApoB, CAC) poderiam ser substituídas por remissões de capítulo para aliviar leitura.

---

## 1. Arcos narrativos dos personagens

### Ricardo (52 → 53 → 54 → 57)
Arco coerente. Cap 1 → 4 → 13 (T+18m pós-IAM) → 15 (T+5 anos). Idades progressivas sem contradição. Entrou com painel "normal" do laboratório + CAC 187 descoberto na emergência, sai 5 anos depois como coautor do próprio plano. **Achado editorial:** a esposa Marina é nomeada pela primeira vez no Cap 13 sem preparação. Caps 1 e 4 não mencionam nenhum familiar. Decisão opcional do autor: adicionar uma menção curta à esposa no Cap 1 ("voltando para casa para a esposa, três filhos adultos") para suavizar a entrada dela em Cap 13.

### Fernanda (41 → 44)
Arco limpo. Cap 4 (baseline), Cap 6 (reversão em 6 meses), Cap 10 (volta aos 44 anos, três anos depois, para decisão de TRH na janela). O salto de 3 anos entre Cap 6 e Cap 10 está marcado no texto ("o trabalho estruturado que fizemos três anos antes seguia firme"). Coerente.

### André (45)
Arco intencionalmente mais curto — é estudo de caso, não arco longitudinal. Cap 3 (idade biológica acelerada) → Cap 6 (Kraft II no TOTG, ajustado em 2026-04-21) → Cap 7 (reestruturação do treino) → Cap 11 (MTHFR, genômica).

### Marcos (57)
Arco coerente. Cap 5 (CAC 412, **rosuvastatina 10 mg**) → Cap 7 (treino) → Cap 8 (alimentação + suplementação + fechamento de 8 meses) → Cap 9 (farmacologia G1) → Cap 15 (placar). A expressão "estatina em dose baixa" aparece em Caps 9, 15 e 17 — clinicamente, rosuvastatina 10 mg é intensidade moderada pelas diretrizes ACC/AHA; o uso narrativo é defensável (em relação ao que seria preciso para atingir ApoB < 60), mas um leitor-médico pode estranhar.

### Paulo (48 → 49.5 → 50)
Arco mais técnico do livro, e funciona bem. Cap 10 (painel baseline) → Cap 11 (6 meses depois, lógica iterativa) → Cap 14 (18 meses, regressão silenciosa + apneia identificada → 24 meses pós-CPAP). Timeline de 4 tempos coerente. A Figura 14.3 resume bem visualmente.

### Ana (44)
Arco da transição bioquímico → psicológico muito bem construído. Cap 12 (Mente Individual — PHQ-9=14, GAD-7=16, caregiver strain da mãe com demência, p-tau217 negativo, fechamento com 6 meses) → Cap 13 (Conexão, Propósito e Sentido — aparição breve como exemplo de caregiver strain). A timeline "sexta consulta em dezoito meses" = consulta a cada 3 meses. Consistente. A mãe com demência é plantada em Cap 12 e floresce em Cap 12 seção Função Cognitiva de forma natural.

**Conclusão dos arcos:** nenhum arco está quebrado. A única melhoria opcional é a menção precoce de Marina.

---

## 2. Estilo e tom

**Consistência:** excelente. Lendo do frontmatter ao Manifesto, não há capítulo que soa escrito por outra pessoa. O tom "médico que conversa com o leitor na consulta" se mantém.

**Algumas observações:**
- Cap 13 (Conexão, Propósito e Sentido) é o mais literário do livro — frases ligeiramente mais longas, metáforas mais ricas ("o corpo estendido ao mundo", "vale de erosão silenciosa"). Intencional e bem-vindo; eleva o livro no ponto onde ele precisa carregar peso emocional.
- Cap 11 (G3) e Cap 9 (G1) são os mais técnicos, com mais listas e citações de ensaios. Apropriado para o conteúdo, mas pode ser cansativo em leitura longa. Solução do autor ("Essencial em 60 Segundos" no final) já mitiga.
- Cap 17 (Manifesto) é a mudança de registro esperada — confessional e em 1ª pessoa forte. Está bem marcado como capítulo à parte.

**Nenhum capítulo soa genérico ou afastado.**

---

## 3. Erros encontrados

### 3.1 Inconsistência entre `personagens.md` e Cap 5 — **RESOLVIDO**
`personagens.md` dizia "rosuvastatina 20 mg" mas Cap 5 diz "rosuvastatina 10 mg por dia". Corrigi `personagens.md` para 10 mg (que é a fonte primária no Cap 5).

### 3.2 Rotulação de Kraft III em André — **RESOLVIDO em 2026-04-21**
Cap 6 foi reescrito com a curva Kraft II: pico de insulina aos 60 min (124 µIU/mL ≈ 14× o basal), descendo a 89 aos 120 min. Rótulo ajustado para "Kraft tipo II". "TOTG em quatro tempos" → "cinco tempos" (0, 30, 60, 90, 120 min) também corrigido. Briefing da figura `Cap06 Fig04` já usa os valores corretos.

### 3.3 "Rosuvastatina 10 mg = estatina em dose baixa" em Caps 9, 15, 17 — **JULGAMENTO DO AUTOR**
Tecnicamente, rosuvastatina 10 mg é intensidade **moderada** pelas diretrizes. O livro usa "baixa" no sentido narrativo (insuficiente para o alvo ApoB 60). Opções:
- (a) Manter como está — o sentido fica claro em contexto.
- (b) Substituir em todas as 3 ocorrências por "dose moderada que ainda estava insuficiente para o alvo" — mais preciso, um pouco mais longo.
- Minha recomendação: (a), mas vale registrar a escolha.

---

## 4. Incoerências

### 4.1 Marina sem apresentação prévia — **PEQUENA LACUNA NARRATIVA**
A esposa de Ricardo aparece nomeada pela primeira vez no Cap 13 ("Com a esposa, Marina, a convivência..."). Caps 1 e 4 não mencionam nenhum familiar, embora o infarto do Cap 1 seja um evento em que esposa/família seriam naturalmente impactados. Não é erro, mas um leitor atento pode sentir que ela surge abrupta.

**Opções:**
- (a) Adicionar uma frase no Cap 1, como *"ligaram para a esposa que chegou no pronto-socorro quarenta minutos depois"* — ancora Marina antecipadamente.
- (b) Manter como está — Cap 13 funciona narrativamente de qualquer jeito.

### 4.2 Idades pós-infarto de Ricardo
Cap 1: 52. Cap 13 (T+18m): ~53.5. Cap 15 (5 anos): 57. Matemática: 52+5=57 (correto), mas 52+1.5=53.5, enquanto o Cap 13 não explicita a idade (só diz "18 meses depois"). Não é incoerência, é omissão intencional de idade exata no Cap 13 — defensável.

### 4.3 Ricardo no Cap 17 (Manifesto)
O Manifesto lista 6 pacientes em sequência curta: *"Ricardo quase morreu num estacionamento... Marcos levou seis anos... Fernanda tinha quarenta e um anos... André treinava dez horas..."* Apresentações coerentes com os capítulos individuais. ✓

### 4.4 Ordem dos sub-pilares I no site × livro
Site (agir-structure.ts) lista I em 5 sub-pilares na ordem: Avaliação Psicológica, Técnicas de Relaxamento, Função Cognitiva, Vida Sexual, Vínculos Sociais. Livro coloca Vida Sexual antes de Vínculos em Cap 13. Checando o site novamente — "Vida Sexual" e "Vínculos Sociais e Suporte" são os dois últimos sub-pilares, nessa ordem. **Está consistente** — Cap 13 segue a mesma ordem (Vida Sexual antes de Vínculos).

---

## 5. Redundâncias desnecessárias

### 5.1 Definição de ApoB
ApoB é definida em Cap 4 (em detalhe), relembrada em Cap 5 (contexto cardiovascular), Cap 6 (metabolismo), Cap 9 (farmacologia de precisão). Cada retomada adiciona contexto, então é defensável — mas em Cap 9 a definição poderia ser substituída por remissão ao Cap 4 para economizar 1-2 frases. **Prioridade: baixa.**

### 5.2 Resumo de Princeton Consensus sobre DE
Cap 12 (no contexto de HPA e sistema nervoso autônomo) não mencionamos DE; Cap 13 (Vida Sexual) traz Princeton Consensus em detalhe. Primeira menção é no Cap 13 — não há redundância.

### 5.3 Repetição das "quatro doenças"
Cap 2 introduz em detalhe. Cap 3, 4, 5, 6 retomam brevemente. Cap 9, 10, 11 usam como "esteira" para os pilares G. Todas as retomadas têm função pedagógica — **não é redundância**; é reforço intencional.

### 5.4 Listas de biomarcadores
Cap 4 dá a lista canônica. Cap 15 (Placar) repete boa parte. A repetição aqui é útil porque o Placar é ferramenta prática — o leitor precisa dos alvos à mão. **Defensável.**

### 5.5 "Janela silenciosa"
Aparece em frontmatter, Cap 1, Cap 2, Cap 4, Cap 15, Cap 17. Cada aparição é em contexto diferente (definição → dramatização → diagnóstico → ferramenta → reflexão). **Reforço intencional bem calibrado.**

**Conclusão das redundâncias:** não há redundâncias desnecessárias relevantes. O livro é disciplinado na economia narrativa.

---

## 6. Bridges e estrutura

### Cap 8 → Cap 9 (A2 → G1)
Cap 8 fecha com *"Os Três Motores em Conjunto"* (Marcos 8 meses depois) + *"E Se Fosse Com Você?"*. Cap 9 abre dizendo: *"A gestão clínica e metabólica é a parte do Método AGIR em que o paciente não pode fazer sozinho."* **Transição clara** — passa do "o que você faz" para "o que precisa de médico".

### Cap 11 → Cap 12 (Genômica → Mente Individual)
Cap 11 fecha com Paulo 6 meses depois + Essencial em 60s + quote: *"A gestão clínica e metabólica, por mais completa que seja, ainda opera na dimensão biológica..."* Cap 12 abre com Ana. **Transição forte** — explícito que a próxima camada é não-biológica.

### Cap 12 → Cap 13 (Mente Individual → Conexão)
Cap 12 fecha com Ana 6 meses depois + quote sobre perguntas que testes não capturam, anuncia Ricardo. Cap 13 abre com Ricardo 18 meses pós-IAM + frase "Eu não estou". **Transição excelente** — um dos melhores momentos estruturais do livro.

### Cap 13 → Cap 14 (Conexão → Ritmo Circadiano)
Cap 13 fecha com Ricardo 12 meses depois (total 30 meses pós-IAM) + quote sobre "há outro sistema, ainda mais fundamental". Cap 14 abre com Paulo 18 meses pós-início. **Transição sólida** — o pilar R como "solo" é uma metáfora forte do livro.

### Cap 14 → Cap 15 (R → Plano)
Cap 14 fecha com Paulo 4 tempos + "O pilar R não é um quinto pilar opcional. É o solo". Cap 15 abre com Ricardo 5 anos depois pedindo ler exames. **Transição eficiente** — mudança de modo (método AGIR encerrado, agora a parte "o que você faz com tudo isso").

### Cap 16 → Cap 17 (Especialista → Manifesto)
Cap 16 fecha com observação sobre pequenos centros × grandes centros + integração. Cap 17 abre com *"Eu tenho a mesma idade biológica que meus pacientes"*. **Transição de modo literário** — explícito para o leitor que vem um registro diferente. ✓

**Pergunta editorial:** Cap 16 está em Parte IV — O Plano. O agente revisor sugeriu movê-lo para Parte V — Encerramento. Defendo manter em Parte IV: "Quando procurar um especialista" é ferramenta prática (parte do plano), não reflexão final. Manter como está.

---

## 7. Prioridades de correção

| Prioridade | Item | Ação sugerida |
|---|---|---|
| ✅ Feito | Rosuvastatina 10 mg vs 20 mg em `personagens.md` | Corrigido durante a revisão |
| **Baixa/opcional** | Marina sem apresentação prévia | Adicionar 1 frase no Cap 1 sobre a esposa — ou deixar como está |
| **Baixa/opcional** | "Estatina em dose baixa" para rosuvastatina 10 mg | Considerar "dose moderada insuficiente para o alvo" — ou manter no sentido narrativo |
| ✅ Feito | Kraft II/III rótulo em André | Resolvido em 2026-04-21 — Cap 6 reescrito com curva Kraft II |
| **Baixa/opcional** | Remissão em vez de redefinição de ApoB em Cap 9 | Economia de leitura de 1–2 frases |
| Nenhuma | Tom, estilo, arcos principais | Publicável como está |

---

## Conclusão

O livro está **apto para publicação** com ajustes pontuais e opcionais. A reestruturação para 18 capítulos produziu um livro melhor-arquitetado do que a versão de 14 capítulos: o pilar I dividido em dois capítulos (mente individual + conexão e sentido) é ganho claro; os três capítulos de gestão clínica (sistemas / painéis / genômica) dão respiro ao que antes era um único capítulo pesado; e a seção de camada visível no Cap 8 resolve uma lacuna (pele/cabelo/unhas + PFS) que faltava.

Dos seis personagens, três têm arco longitudinal completo (Fernanda, Paulo, Ricardo), dois têm arco parcial didaticamente adequado (Marcos, Ana), e um é estudo de caso pontual (André). A decisão de não seguir André longitudinalmente é coerente com a função dele no livro — ilustrar overtraining, não demonstrar recuperação.

Os bridges entre capítulos são entre as partes melhor executadas do livro. O tom autoral é consistente. As redundâncias existentes são pedagógicas, não descuidadas.

**Em síntese:** não há achado que bloqueie publicação. As cinco pendências de prioridade baixa podem ser endereçadas em uma rodada de polimento de ~1h de trabalho editorial, ou parcialmente deixadas para uma segunda edição.

---

*Revisão realizada por delegação parcial a agente de exploração + verificação manual de pontos críticos. Achados consolidados por Claude.*

#!/usr/bin/env python3
"""Build print-ready miolo PDF for 'ANTES' — Brazilian HARDCOVER printer (capa dura).

Usage:
    python3 build-hardcover-miolo.py pt-BR

Premium edition pipeline:
    - Trim 16 × 23 cm with 0,25 cm sangria all sides → page 16,5 × 23,5 cm.
    - 0,5 cm minimum safety margin from trim line.
    - FULL COLOR miolo. Author-edition palette (Dr. Getúlio gold + ink),
      explicitly NOT Plenya petrol/gold — that lives on the cover only.
    - Images copied byte-for-byte from source (shutil.copy2). No grayscale
      conversion, no re-compression, no resampling. Whatever resolution
      and format the source has is what the PDF embeds — xelatex's
      \\includegraphics passes the file straight through to the PDF
      stream as-is.
    - No crop marks (printer asked CRUZETAs removed; xelatex omits them
      by default — confirmed).
    - Output to build/capa-dura/ subfolder.
"""
...
from PIL import Image  # still used by audit_image_dpi()

THIS_DIR  = Path(__file__).resolve().parent           # …/build/capa-dura
BUILD_DIR = THIS_DIR.parent                            # …/build
BOOK_ROOT = BUILD_DIR.parent                           # …/Ebook 1 …
...
def prepare_images(out_dir):
    """Copy figures byte-for-byte — zero re-encoding for premium quality.

    No PIL pipeline: shutil.copy2 preserves the source bits exactly,
    including ICC profile, alpha channel, color depth and metadata.
    xelatex's \\includegraphics embeds the file as-is into the PDF
    stream, so the only quality factor is the source resolution.
    Filenames are normalised (spaces → underscores) so the lua image
    path rewriter can resolve them inside the work dir.
    """
...
def audit_image_dpi(img_dir, text_width_in=4.92, soft_dpi=300, hard_dpi=200):
    # text_width_in=4.92" ≈ 125 mm = the printable width on this 16×23 cm trim
    # (165 mm page − 22 mm inner − 18 mm outer = 125 mm).
    """Audit figures vs print DPI.

    Three categories:
      OK    : DPI >= soft_dpi at full text-width (4.92\")           — clean print
...
def main():
    print(f"📖 Building HARDCOVER miolo PDF for language: {LANG}")
    ...
    print(f"   DPI audit (at 4.92\" text width):")

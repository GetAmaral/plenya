#!/usr/bin/env python3
"""Build print-ready miolo PDF for 'ANTES' — BROCHURA COM ORELHAS (gráfica nacional).

Terceira edição física, ao lado do paperback KDP (6×9") e da capa dura:

  - Trim 16 × 23 cm com 2,5 mm de sangria (mesmo formato da capa dura).
  - Miolo P&B: reaproveita integralmente os overrides vetoriais e as tabelas
    nativas de ``versaoImpressa/`` — os assets não são duplicados, este script
    aponta para lá.
  - Corpo 11pt/1,10 (a medida de linha aqui é 125 mm, mais larga que a do 6×9").
  - Capa com orelhas de 8 cm, montada por ``build-brochura-cover.py``.

Herdado de versaoImpressa/build-print-pdf.py:
...
Usage:
    ./versaoImpressa/runpy brochura/build-brochura-miolo.py pt-BR
"""
...
BUILD_DIR = Path(__file__).resolve().parent             # build/brochura/
BOOK_ROOT = BUILD_DIR.parent.parent                     # Ebook 1.../
_args = [a for a in sys.argv[1:] if not a.startswith("--")]
_flags = {a for a in sys.argv[1:] if a.startswith("--")}
LANG = _args[0] if _args else "pt-BR"

# Variante de cor. Duas provas físicas da mesma brochura:
#   --pb   tipografia em cinza  + figuras em cinza   (100% preto e branco)
#   --cor  tipografia em petrol + figuras coloridas  (default histórico = --meio,
#          tipografia colorida com figuras em cinza)
# A paginação é idêntica nas três: cor não muda métrica de texto nem bounding
# box de figura, então a lombada e a capa servem para qualquer uma.
VARIANT = "pb" if "--pb" in _flags else ("cor" if "--cor" in _flags else "meio")
...
FIG_BW_DIR   = BUILD_DIR.parent / "versaoImpressa" / "figuras-bw"   # overrides B&W (compartilhados)
FIG_COR_DIR  = BUILD_DIR.parent / "versaoImpressa" / "figuras-cor"  # overrides coloridos
TABLES_DIR   = BUILD_DIR.parent / "versaoImpressa" / "tabelas-nativas"  # tabelas vetoriais (compartilhadas)
AUTHOR_PHOTO = BOOK_ROOT / "fotos" / "getulio_bw_halfbody_fullres.jpg"
WORK_DIR     = BUILD_DIR / f"work-brochura-{LANG}-{VARIANT}"
...
def prepare_images(out_dir):
    ...
    # Na variante colorida os overrides vêm de figuras-cor/; o que não existir
    # lá cai no PNG colorido original, que é justamente o desejado para as
    # quatro figuras de imagem médica (TC, ultrassom, cortes anatômicos).
    _override_dir = FIG_COR_DIR if VARIANT == "cor" else FIG_BW_DIR
    if _override_dir.exists():
        for f in _override_dir.iterdir():
...
def audit_image_dpi(img_dir, text_width_in=4.92, soft_dpi=300, hard_dpi=200):
...
% --- Contagem contínua de páginas -------------------------------------------
% Regra apontada pelo editor em 2026-08-26: "a numeração do livro é contada
% desde a primeira página, mesmo que ela seja em branco". O padrão do book
% (\frontmatter em romanos + \mainmatter reiniciando em 1) fazia a Introdução
% imprimir "1" na 13ª folha física. Aqui a contagem segue de ponta a ponta: o
% miolo continua de onde a folha de rosto parou, e o fólio só começa a ser
% IMPRESSO no corpo (o pré-textual segue com \pagestyle{empty}).
% \value{page} aqui vale a próxima folha a ser composta, então não depende do
% tamanho do pré-textual — se um branco entrar ou sair, a conta se ajusta.
\newcount\primeirafolhadocorpo
\primeirafolhadocorpo=\value{page}
\mainmatter
\setcounter{page}{\the\primeirafolhadocorpo}
\pagestyle{fancy}
```
"""
    return fm
...
def pad_to_signature(pdf_path, multiple=4):
    """Completa o miolo com páginas em branco até um múltiplo de `multiple`.

    Gráfica trabalha em cadernos: um PDF com número de páginas fora do múltiplo
    de 4 volta da pré-impressão ou recebe folhas em branco em posição arbitrária.
    Melhor decidir aqui, no fim do miolo, onde a página em branco é invisível.

    Usa pikepdf. Se o interpretador atual não tiver a lib (o venv de figuras não
    tem), tenta de novo no python3 do sistema antes de desistir.
    """
    script = (
        "import sys, pikepdf\n"
        "p, mult = sys.argv[1], int(sys.argv[2])\n"
        "pdf = pikepdf.open(p, allow_overwriting_input=True)\n"
        "n = len(pdf.pages); falta = (-n) % mult\n"
        "if falta:\n"
        "    mb = pdf.pages[-1].mediabox\n"
        "    size = (float(mb[2]) - float(mb[0]), float(mb[3]) - float(mb[1]))\n"
        "    [pdf.add_blank_page(page_size=size) for _ in range(falta)]\n"
        "    pdf.save()\n"
        "print(f'{n} {falta}')\n"
    )
    try:
        out = subprocess.run([sys.executable, "-c", script, str(pdf_path), str(multiple)],
                             capture_output=True, text=True)
        if out.returncode != 0:
            out = subprocess.run(["python3", "-c", script, str(pdf_path), str(multiple)],
                                 capture_output=True, text=True)
        if out.returncode != 0:
            print(f"   ⚠ Não foi possível completar cadernos (pikepdf ausente): {out.stderr.strip().splitlines()[-1:]}")
            return None
        n, falta = (int(v) for v in out.stdout.split())
    except Exception as exc:                                    # noqa: BLE001
        print(f"   ⚠ Não foi possível completar cadernos: {exc}")
        return None

    if falta:
        print(f"   Cadernos: {n} → {n + falta} páginas (+{falta} em branco no fim, múltiplo de {multiple})")
    else:
        print(f"   Cadernos: {n} páginas já é múltiplo de {multiple}")
    return n + falta
...
def main():
    ...
    print(f"   DPI audit (a 4.92\" largura de texto):")

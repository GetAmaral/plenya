...
% O polyglossia transforma cada hífen num \discretionary{-}{-}{-} e REPETE o
% hífen na linha seguinte (splithyphens, ligado por padrão). Em palavra
% composta portuguesa essa repetição é a regra do Acordo Ortográfico e fica.
% Desligar aqui, global, custava caro: sem o discretionary o
% \exhyphenpenalty=10000 abaixo proíbe qualquer quebra em hífen existente, e
% um DOI passava 34 pt para fora da margem. O ajuste é local, no capítulo de
% Referências, onde estão os títulos em inglês e os DOIs — ver o bloco
% {=latex} no topo de md/pt-BR/18-referencias-recursos.md.
...
% {gray} e não {HTML}: em miolo P&B o preto tem de sair de UMA chapa. Definidas
% em HTML, estas três cores viravam DeviceRGB — 2.824 operadores rg/RG em 321 das
% 344 páginas, inclusive o cabeço de 8,25 pt de toda página. Neutro em RGB
% (R=G=B) ainda é RGB: numa conversão para CMYK sem preservar preto vira preto de
% quatro cores, e texto pequeno em quatro chapas dá halo de registro. Os valores
% de cinza abaixo são os mesmos tons de antes (17, 118 e 26 em 255).
\definecolor{petrol}{gray}{0.067}    % títulos, capitulares, rótulo de legenda
\definecolor{gold}{gray}{0.463}      % numeração "PARTE N" / "CAPÍTULO N"
\definecolor{ink}{gray}{0.102}       % corpo — inalterado
...
% --- Headers / footers ---
% A folha que o \cleardoublepage insere para o capítulo abrir em ímpar saía
% com cabeço e fólio ("40 · ANTES" numa página vazia). O \cleardoublepage do
% LaTeX base não aplica \thispagestyle{empty}; o emptypage aplica.
...
% Quando não há \chaptermark (Introdução, textos de abertura de parte) o
% \leftmark vem vazio e o cabeço saía como "· 15", com o separador solto.
\makeatletter
\newcommand{\plenyaheadmark}{%
  \protected@edef\@tempa{\leftmark}%
  \ifx\@tempa\@empty\else
    \nouppercase{\leftmark}\quad\textcolor{gray}{·}\quad
  \fi}
\makeatother
\fancyhead[RO]{\sffamily\footnotesize\color{ink}\plenyaheadmark\thepage}
...
% O corte era só no travessão, então os quatro capítulos cujo título usa
% dois-pontos (4, 6, 15, 17) iam inteiros para o cabeço — o do 4 ocupava
% 280 dos 337 pt da medida, o dobro dos outros. Corta nos dois.
\makeatletter
\def\plenya@cutdash#1 — #2\@nil{\def\plenya@mark{#1}}
\def\plenya@cutcolon#1: #2\@nil{\def\plenya@mark{#1}}
\renewcommand{\chaptermark}[1]{%
  \plenya@cutdash#1 — \@nil
  \expandafter\plenya@cutcolon\plenya@mark: \@nil
  \markboth{\plenya@mark}{}}
\makeatother
...
%   finalhyphendemerits=100000 (era 10000) desencoraja a quebra que deixa o
%     rabo da palavra sozinho na última linha do parágrafo. Sozinho mexe
%     pouco (11 casos → 10); o que resolveu o pior deles, "Suplementação
%     Inteli-" / "gente" nas p.120 e p.137, foi brochura/linha-de-destaque.lua,
%     que trata a linha "***Pilar X — ...***" como título e não a hifeniza.

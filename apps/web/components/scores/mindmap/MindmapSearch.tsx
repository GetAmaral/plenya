...
export function MindmapSearch({ scoreGroups, isOpen, onClose, onResultClick }: MindmapSearchProps) {
    ...
    scoreGroups.forEach(group => {
      ...
      group.subgroups?.forEach(subgroup => {
        ...
        subgroup.items?.forEach(item => {
          if (normalizeForSearch(item.name).includes(query)) {
            results.push({
              type: 'item',
              id: `item-${item.id}`,
              name: item.name,
  ...
  return (
    <div className="absolute top-20 left-4 z-50 w-[480px] bg-background border rounded-lg shadow-2xl">
      {/* Header */}

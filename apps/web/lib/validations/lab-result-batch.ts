...
export const labResultInBatchSchema = z.object({
  ...
  testName: z
    .string()
    .min(2, "Nome do exame deve ter no mínimo 2 caracteres")
    .max(200, "Nome do exame deve ter no máximo 200 caracteres"),
  testType: z
    .string()
    .min(2, "Tipo do exame deve ter no mínimo 2 caracteres")
    .max(100, "Tipo do exame deve ter no máximo 100 caracteres"),
...
// Schema para o batch completo
export const labResultBatchSchema = z.object({
  ...
  (data) => data.labResults.some(r => r.testName && r.testName.trim()),
  {
    message: "Adicione pelo menos um resultado preenchido ao lote",
    path: ["labResults"],
  }
);

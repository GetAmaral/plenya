...
export interface FamilyGrantView {
  id: string;
  patientId: string;
  granteeEmail: string;
  granteeLabel: string;
  granteeUserId?: string | null;
  scope: FamilyAccessScope;
...
export const familyAccessApi = {
  list: () => apiClient.get<FamilyGrantView[]>("/api/v1/patient/me/family"),
  create: (payload: {
    granteeEmail: string;
    granteeLabel: string;
    scope: FamilyAccessScope;
  }) => apiClient.post<FamilyGrantView>("/api/v1/patient/me/family", payload),
  updateScope: (id: string, scope: FamilyAccessScope) =>
    apiClient.put<void>(`/api/v1/patient/me/family/${id}/scope`, { scope }),
  revoke: (id: string) => apiClient.delete<void>(`/api/v1/patient/me/family/${id}`),
  consume: (token: string) =>
    apiClient.post<FamilyGrantView>("/api/v1/patient/me/family/consume", { token }),
  grantedToMe: () =>
    apiClient.get<FamilyGrantView[]>("/api/v1/patient/me/family/granted-to-me"),
};

export function useFamilyGrants() {
  return useQuery({ queryKey: ["family-grants"], queryFn: familyAccessApi.list });
}

export function useCreateFamilyInvite() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: familyAccessApi.create,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["family-grants"] }),
  });
}

export function useUpdateFamilyScope() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, scope }: { id: string; scope: FamilyAccessScope }) =>
      familyAccessApi.updateScope(id, scope),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["family-grants"] }),
  });
}
...
export function useRevokeFamilyGrant() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: familyAccessApi.revoke,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["family-grants"] }),
  });
}

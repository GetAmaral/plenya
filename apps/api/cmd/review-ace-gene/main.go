...
// Review ACE I/D rs4646994 (Hipertensão) - Revisão completa
// Inclui busca RAG, PubMed com query específica para ACE gene polymorphism

func main() {
	scoreItemID := "019c1a2b-a36f-7f65-af8c-510bf0b858e0"
	itemUUID := uuid.MustParse(scoreItemID)

	fmt.Printf("🔍 Reviewing ScoreItem: ACE I/D rs4646994 (Hipertensão)\n")
	...
	if currentArticleCount < int64(minArticles) {
		...
		// Query específica para ACE I/D polymorphism e hipertensão
		query := "(ACE gene[Title/Abstract] OR angiotensin-converting enzyme[Title/Abstract]) AND (polymorphism[Title/Abstract] OR rs4646994[Title/Abstract] OR insertion deletion[Title/Abstract]) AND (hypertension[MeSH Terms] OR cardiovascular[Title/Abstract] OR blood pressure[Title/Abstract]) AND (Review[ptyp] OR Meta-Analysis[ptyp])"

		fmt.Printf("PubMed query: %s\n\n", query)
	...
	if item.ClinicalRelevance != nil {
		...
		fmt.Printf("   Preview: %s\n", truncate(*item.ClinicalRelevance, 150))
	}

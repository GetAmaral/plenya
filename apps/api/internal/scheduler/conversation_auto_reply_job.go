...
func (j *ConversationAutoReplyJob) handleConversation(ctx context.Context, ownerType string, ownerID uuid.UUID, settings *models.ReceptionSettings) {
	...
	if answered > 0 {
		...
		j.convSvc.MaintainRelationshipMemory(ctx, ownerType, ownerID, true)
		j.clearDraft(ctx, ownerType, ownerID)
		return
	}

	// Memória de longo prazo (social): regenera resumo + fatos a cada +5 mensagens novas.
	// Best-effort, gatilho interno barato (só chama a IA quando acumulou o suficiente). Guardrail §3.1.
	j.convSvc.MaintainRelationshipMemory(ctx, ownerType, ownerID, false)

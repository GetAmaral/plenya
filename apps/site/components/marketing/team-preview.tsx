import Image from 'next/image';
import { Link } from '@/lib/i18n/navigation';

export function TeamPreview() {
  return (
    <section className="bg-cream">
      <div className="site-container section grid lg:grid-cols-[1fr_auto] gap-12 lg:gap-20 items-center">
        <div className="space-y-6 max-w-xl">
          <p className="label-upper text-gold">Equipe</p>
          <h2 className="heading-section text-petrol text-3xl md:text-5xl">
            Quem conduz a sua trajetória.
          </h2>
          <p className="text-petrol/80 text-lg leading-relaxed">
            Médicos, nutricionista, psicóloga e educador físico falando a mesma
            língua — sobre o mesmo painel, sobre a mesma pessoa. A equipe se
            reúne, discute o seu caso e desenha uma conduta única.
          </p>
          <p className="text-petrol/70 leading-relaxed">
            Não são especialidades operando em paralelo. É uma leitura clínica
            integrada, traduzida em prescrição compartilhada e revisada a cada
            ciclo.
          </p>
          <div className="pt-4">
            <Link href="/equipe" className="btn-outline-dark">
              Conhecer a equipe Plenya
            </Link>
          </div>
        </div>
        <div className="relative w-full max-w-md lg:w-[380px] lg:max-w-none aspect-[1064/1891] overflow-hidden bg-petrol/5">
          <Image
            src="/images/team/equipe-formal.jpg"
            alt="Equipe Plenya"
            fill
            className="object-cover object-top"
            sizes="(min-width: 1024px) 380px, 100vw"
          />
        </div>
      </div>
    </section>
  );
}

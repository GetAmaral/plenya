...
-- ============================================================================
-- STEP 2: INSERT SCIENTIFIC ARTICLES (Check first to avoid duplicates)
-- ============================================================================
...
SELECT
    ...
    'en'
WHERE NOT EXISTS (SELECT 1 FROM articles WHERE pm_id = '39256087');
...
SELECT
    ...
    'en'
WHERE NOT EXISTS (SELECT 1 FROM articles WHERE pm_id = '37000150');

-- Article 3: Serum Cholesterol and CVD Death Meta-Analysis (2022)
-- This one already exists (35886124), so skip
...
SELECT
    ...
    'en'
WHERE NOT EXISTS (SELECT 1 FROM articles WHERE pm_id = '37833089');
...
WHERE a.pm_id IN ('39256087', '37000150', '35886124', '37833089')
  AND NOT EXISTS (
    ...
    WHERE asi.article_id = a.id
      AND asi.score_item_id = 'd17cea8f-2935-40bc-b2e0-bcd9ac886ade'
  );
...
BEGIN
    ...
    RAISE NOTICE 'Articles in database: %/4', v_article_count;
    ...
    IF v_article_count >= 3 AND v_link_count >= 3 THEN

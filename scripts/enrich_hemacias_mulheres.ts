...
async function searchArticles(anthropic: Anthropic): Promise<string> {
  console.log('\n' + '='.repeat(80));
...
Liste os artigos com título, autores, DOI e principais achados.`;
...
async function saveArticles(
  ...
  let currentArticle: { title?: string; doi?: string; content?: string } = {};

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();

    // Detectar título
    if (line.match(/^(Title|Título|Article|1\.|2\.|3\.)/i)) {
    ...
    // Acumular conteúdo
    if (line && !line.match(/^(Title|DOI|PMID|Authors?)/i)) {
...
async function saveArticle(
  dbClient: Client,
  article: { title?: string; doi?: string; content?: string },
  articleIds: string[]
) {
...
async function updateScoreItem(dbClient: Client, enrichedData: EnrichedData) {
  console.log('\n' + '='.repeat(80));

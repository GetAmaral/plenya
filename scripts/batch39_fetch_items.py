#!/usr/bin/env python3
"""
Busca os 30 itens de Composição Corporal da API
"""
...
for idx, item_id in enumerate(ITEM_IDS, 1):
    print(f"[{idx}/30] Buscando {item_id}...")
    ...
    if response.status_code == 200:
        ...
        items.append({
            ...
            "has_content": len(item.get("clinical_context", "")) > 100
        })
...
# Salva resultado
with open("/home/user/plenya/batch39_items.json", "w", encoding="utf-8") as f:
    json.dump(items, f, ensure_ascii=False, indent=2)

print(f"\n✅ {len(items)} itens salvos em batch39_items.json")
print(f"   Com conteúdo: {sum(1 for i in items if i['has_content'])}")
print(f"   Sem conteúdo: {sum(1 for i in items if not i['has_content'])}")

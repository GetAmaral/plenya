...
-- 2. Inserir artigos científicos (verificar se já existem por DOI)
...
WITH new_article AS (
  ...
  SELECT
    ...
    'review'
  WHERE NOT EXISTS (
    SELECT 1 FROM articles WHERE doi = '10.1016/j.biopha.2022.113261'
  )
  RETURNING id
)
INSERT INTO article_score_items (article_id, score_item_id)
SELECT id, 'c21ccec2-66b2-49e3-911a-8d0944eda087'::uuid
FROM new_article
WHERE NOT EXISTS (
  SELECT 1 FROM article_score_items asi
  JOIN articles a ON asi.article_id = a.id
  WHERE a.doi = '10.1016/j.biopha.2022.113261'
    AND asi.score_item_id = 'c21ccec2-66b2-49e3-911a-8d0944eda087'::uuid
);

-- Se artigo já existe, criar associação
INSERT INTO article_score_items (article_id, score_item_id)
...
WITH new_article AS (
  ...
  SELECT
    ...
    'editorial'
  WHERE NOT EXISTS (
    SELECT 1 FROM articles WHERE doi = '10.1055/s-0042-1748036'
  )
  RETURNING id
)
INSERT INTO article_score_items (article_id, score_item_id)
...
WHERE NOT EXISTS (
  SELECT 1 FROM article_score_items asi
  JOIN articles a ON asi.article_id = a.id
  WHERE a.doi = '10.1055/s-0042-1748036'
    AND asi.score_item_id = 'c21ccec2-66b2-49e3-911a-8d0944eda087'::uuid
);
...
WITH new_article AS (
  ...
  SELECT
    ...
    'review'
  WHERE NOT EXISTS (
    SELECT 1 FROM articles WHERE doi = '10.3390/ijms21218191'
  )
  RETURNING id
)
INSERT INTO article_score_items (article_id, score_item_id)
...
WHERE NOT EXISTS (
  SELECT 1 FROM article_score_items asi
  JOIN articles a ON asi.article_id = a.id
  WHERE a.doi = '10.3390/ijms21218191'
    AND asi.score_item_id = 'c21ccec2-66b2-49e3-911a-8d0944eda087'::uuid
);
...
-- Verificação de contagem de caracteres
SELECT
...
-- Verificar artigos vinculados (incluindo os novos)
SELECT

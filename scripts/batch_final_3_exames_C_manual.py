...
Geração manual de SQL com conteúdo MFI pré-definido
"""

import json
from pathlib import Path

# Conteúdo MFI enriquecido para os 35 itens
ENRICHMENTS = [
    # 1. Fundoscopia - Retinopatia Diabética
    {
        "item_id": "dbbb0258-ebcb-4bb2-ace3-73d0d7b93a52",
        "optimal_range_min": None,
        "optimal_range_max": None,
        "unit": "grau (0-4)",
...
- Monitorar pressão arterial domiciliar diariamente""",
        "related_articles": [
            "Wong TY, et al. Diabetic retinopathy. Nat Rev Dis Primers. 2016;2:16012. PMID: 27159554",
            "Simó R, et al. Neurodegeneration in diabetic retinopathy: does it really matter? Diabetologia. 2018;61(9):1902-1912. PMID: 29392494",
            "Stern JH, et al. Emerging therapies for diabetic retinopathy. Curr Opin Ophthalmol. 2023;34(3):214-220. PMID: 36938849"
        ]
    },
    # 2. USG Transvaginal - Espessura Endometrial Pós-Menopausa
    {
        "item_id": "30af9809-7316-47e6-b363-8279c7bd3a69",
        "optimal_range_min": None,
...
- Atenção a sangramento pós-menopausa (sempre investigar)""",
        "related_articles": [
            "Smith-Bindman R, et al. Endometrial thickness for diagnosing endometrial cancer in symptomatic postmenopausal women. Obstet Gynecol. 2020;135(2):e16-e36. PMID: 31977782",
            "Clarke MA, et al. Association of endometrial cancer risk with postmenopausal bleeding. JAMA Intern Med. 2018;178(9):1210-1222. PMID: 30083701",
            "Giannella L, et al. Accuracy of ultrasound in predicting endometrial pathology. J Minim Invasive Gynecol. 2023;30(4):289-297. PMID: 36581012"
        ]
    },
    # 3. USG Transvaginal - Volume Ovariano
    {
        "item_id": "afdd9d67-3f42-4e6e-a77d-0e75475ca72d",
...
- Pós-menopausa: USG anual se volume normal, imediato se alteração""",
        "related_articles": [
            "Balen AH, et al. Ultrasound assessment of the polycystic ovary: international consensus definitions. Hum Reprod Update. 2003;9(6):505-514. PMID: 14714587",
            "Souter I, et al. Ovarian volume and antral follicle count in women with polycystic ovary syndrome. Fertil Steril. 2021;115(6):1478-1485. PMID: 33832672",
            "Amer SA, et al. Ultrasonographic evaluation of ovarian reserve in women of reproductive age. Ultrasound Obstet Gynecol. 2022;60(3):291-301. PMID: 35191089"
        ]
    },
    # 4. ECG - QTc (Bazett) - Homens
    {
        "item_id": "3a0d7e6b-c53d-47de-a50c-d7774e542835",
...
- QTc prolongado grave: Holter 24h, ECG semanal inicial""",
        "related_articles": [
            "Rautaharju PM, et al. AHA/ACCF/HRS recommendations for the standardization and interpretation of the electrocardiogram. Circulation. 2009;119(10):e241-250. PMID: 19228821",
            "Vandenberk B, et al. Which QT correction formulae to use for QT monitoring? J Am Heart Assoc. 2016;5(6):e003264. PMID: 27317349",
            "Priori SG, et al. 2015 ESC Guidelines for management of patients with ventricular arrhythmias and prevention of sudden cardiac death. Eur Heart J. 2015;36(41):2793-2867. PMID: 26320108"
        ]
    },
...
- Após correção eletrolítica: ECG 7-14 dias""",
        "related_articles": [
            "Rautaharju PM, et al. Sex differences in the evolution of the electrocardiographic QT interval with age. Can J Cardiol. 1992;8(7):690-695. PMID: 1422988",
            "Sedlak T, et al. Sex hormones and the QT interval: a review. J Womens Health. 2012;21(9):933-941. PMID: 22663191",
            "Bai CX, et al. Distinct effects of 17β-estradiol on cardiac repolarization. Circulation. 2005;111(11):1365-1373. PMID: 15753228"
        ]
    }
]

def escape_sql(text):
    """Escapa strings para PostgreSQL"""
    if text is None:
        return "NULL"
    return "'" + str(text).replace("'", "''").replace("\\", "\\\\") + "'"

def format_array(articles):
    """Formata array de artigos para PostgreSQL"""
    if not articles:
        return "NULL"
    escaped = [escape_sql(art)[1:-1] for art in articles]  # Remove aspas externas
    return "ARRAY[" + ", ".join([f"'{art}'" for art in escaped]) + "]"

def generate_sql(enrichments):
    """Gera SQL com os enrichments"""
    lines = [
...
UPDATE score_items
SET
  optimal_range_min = {enr['optimal_range_min'] if enr['optimal_range_min'] is not None else 'NULL'},
  optimal_range_max = {enr['optimal_range_max'] if enr['optimal_range_max'] is not None else 'NULL'},
  unit = {escape_sql(enr['unit'])},
  optimal_explanation = {escape_sql(enr['optimal_explanation'])},
  clinical_interpretation = {escape_sql(enr['clinical_interpretation'])},
  medical_conduct = {escape_sql(enr['medical_conduct'])},
  related_articles = {format_array(enr['related_articles'])},
  updated_at = NOW()
...
""")
    ...
    lines.extend([
        ...
        "COMMIT;",
        "",
        f"-- Total de items atualizados: {len(enrichments)}",
        ""
    ])
...
def main():
    print("=" * 80)
    print("BATCH FINAL 3 - EXAMES C (Primeiros 5 items)")
    print("=" * 80)
    print()
    ...
    # Salvar SQL
    output_file = Path(__file__).parent / "batch_final_3_exames_C_part1.sql"
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(sql_content)

    print(f"✓ SQL gerado: {output_file}")
    print(f"✓ Items processados: {len(ENRICHMENTS)}")
    print()
    print("PRÓXIMO PASSO:")

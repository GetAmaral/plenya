...
INSERT INTO articles (
    ...
    'research_article',
    'Pesquisa de 2025 demonstrando que respostas de IgA em centros germinativos são essenciais para prevenir colonização viral crônica do intestino. Na ausência de IgA, colonização viral crônica desencadeia expansão de linfócitos intraepiteliais CD8αβ+ direcionada por antígenos. Principais achados: IgA é essencial para prevenir colonização viral crônica intestinal; Ausência de IgA permite colonização viral persistente; Colonização viral crônica sem IgA desencadeia expansão de células T CD8+; IgA mantém homeostase imunológica controlando vírus intestinais.',
    'Imunologia',
    NOW(),
    NOW()
);
...
SELECT
    '5230ca12-d4dc-4d1c-8ee8-e8b8b3b23a23'::uuid,
    id
FROM articles
WHERE title IN (
    'IgA deficiency destabilizes homeostasis toward intestinal microbes and increases systemic immune dysregulation',
...
SELECT
    ...
    si.last_review,
    COUNT(asa.article_id) as linked_articles_count
FROM score_items si
LEFT JOIN article_score_items asa ON si.id = asa.score_item_id

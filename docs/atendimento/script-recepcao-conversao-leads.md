# Script de Atendimento da Recepção Plenya

> Roteiro de atendimento e conversão de leads para a recepção da Plenya, nos canais **WhatsApp** e **telefone**.
> Meta: transformar o contato em uma **Consulta Plenya agendada**, plantando o Continuum sem fechar preço.
> Voz da marca: prosa clínica, calma, conectiva. Sem pressão, sem promessa, sem jargão de vendas.

---

## 0. Antes de começar (leia uma vez)

**O que a recepção vende:** a porta de entrada é a **Consulta Plenya** (particular, paga). É nela que o
paciente conhece o Dr. Getúlio e a medicina da casa. O programa **Continuum** (acompanhamento de 6 ou 12
meses com a equipe) é apresentado depois, pelo médico, na própria consulta. A recepção **não fecha o
Continuum nem fala valor dele**. A recepção agenda a consulta e deixa a porta do Continuum aberta.

**Condições da consulta (fixas):**

| Dado | Valor |
|---|---|
| **Valor da Consulta Plenya** | **R$ 800,00**, valor único e não negociável |
| Formas de pagamento | À vista: Pix ou cartão |
| Quando se paga | Antecipado, antes do início da consulta |

> O valor é fixo e não negociável. Não há desconto, cortesia nem negociação de preço. A recepção
> informa o valor com naturalidade e firmeza gentil. Não falar de Continuum nem de qualquer valor a
> ele ligado: a recepção trata só da consulta.

### Dados da clínica (use exatamente assim)

| Campo | Valor |
|---|---|
| Nome | Plenya |
| Endereço de atendimento | Av. Ayrton Senna da Silva, 500, Edifício Torre Pietra, sala 1402, Gleba Palhano, Londrina/PR, 86050-460 |
| Telefone / WhatsApp | (43) 99974-8899 |
| E-mail | contato@plenyasaude.com.br |
| Site | plenyasaude.com.br |
| Instagram | @plenyaSaude |
| Horário | Segunda a sexta, 8h às 18h |
| Direção clínica | Dr. Getúlio Amaral Filho, médico nefrologista (CRM-PR 21.876, RQE 16.038) |

### Sobre a Consulta (o que você precisa saber de cor)

- **Duração:** em média 60 minutos. É uma avaliação completa, não uma consulta rápida de rotina.
- **Formato:** presencial em Londrina ou online por telemedicina, com a mesma profundidade.
- **Pagamento:** particular. A clínica não trabalha com convênios. Despesas médicas são dedutíveis no
  Imposto de Renda como gasto com saúde.
- **O que o paciente traz:** exames anteriores (laboratoriais, de imagem, laudos antigos) e a lista de
  medicamentos em uso. Quanto mais histórico, melhor a leitura.
- **O que a consulta entrega:** escuta longa, leitura do painel laboratorial ampliado, e uma conduta clara
  organizada pelo Método AGIR. Pode incluir pedido de novos exames quando há indicação.

---

## 1. Princípios de atendimento

Seis princípios que valem em toda conversa, por qualquer canal.

1. **Escutar antes de oferecer.** A pessoa liga ou escreve por um motivo. Descubra o motivo antes de
   apresentar a consulta. Quem oferece sem entender empurra; quem entende primeiro conduz.

2. **Espelhar o momento da pessoa.** Existem seis situações típicas de quem procura a Plenya (seção 8).
   Reconheça qual é a da pessoa e devolva isso com as palavras dela. "Pelo que você me conta, parece que
   está naquele momento de..." cria reconhecimento, e reconhecimento converte.

3. **Conduzir, não pressionar.** Cada etapa termina com um próximo passo claro e fácil. Você guia a
   conversa para o agendamento, sem forçar. A pressa assusta o público da Plenya.

4. **Clareza acima de tudo.** Se a Plenya não é para aquela pessoa naquele momento, diga com tranquilidade.
   Dizer não com clareza também é cuidado, e protege a reputação da clínica. Curioso sem intenção não é lead
   perdido, é agenda preservada.

5. **Próximo passo sempre concreto.** Nenhuma conversa termina no ar. Ou agenda, ou combina um retorno com
   data, ou registra o contato para reativar. "Qualquer coisa estou à disposição" não é fechamento.

6. **Tom da casa.** Frases inteiras e conectadas, calmas, sem gírias e sem exclamação em excesso. Você
   representa uma clínica de medicina séria, não um televendas. Nunca prometa cura, resultado ou diagnóstico.

---

## 2. Mapa da conversa

A mesma espinha vale para WhatsApp e telefone. As etapas não são um interrogatório; fluem naturalmente.

```
1. ACOLHIMENTO        Receber, identificar, criar conforto.
2. DESCOBERTA         Entender o motivo do contato e o momento da pessoa (qual dos 6 perfis).
3. VALOR              Conectar o que a pessoa trouxe ao que a Consulta Plenya entrega.
4. PLANTAR CONTINUUM  Mencionar de leve que existe um acompanhamento maior, sem fechar.
5. OBJEÇÕES           Acolher a dúvida, responder com clareza, voltar ao próximo passo.
6. AGENDAMENTO        Marcar dia, formato e coletar os dados. Confirmar o combinado.
7. PÓS                Confirmar com antecedência, lembrar, preparar a consulta.
```

Regra de ouro do fluxo: **você só fala de valor (etapa 3) depois de ter escutado (etapa 2).** Quem pula a
descoberta vende no escuro.

---

## 3. Roteiro WhatsApp (mensagens prontas)

Mensagens curtas, uma ideia por mensagem, tom escrito e gentil. Adapte os colchetes. Nunca dispare tudo de
uma vez: escreva, espere a resposta, siga.

### 3.1 Abertura por origem do lead

**A) Veio do formulário do site / aplicação**
```
Olá, [nome]. Aqui é a [seu nome], da Plenya.
Recebi seu contato pelo nosso site, obrigada por procurar a gente.
Para eu te orientar do jeito certo, posso te fazer duas ou três perguntas rápidas?
```

**B) Veio da DM do Instagram**
```
Oi, [nome], tudo bem? Aqui é a [seu nome], da Plenya.
Vi sua mensagem por aqui. Vou te responder com calma.
Me conta o que te trouxe até a gente?
```

**C) Indicação de paciente/conhecido**
```
Olá, [nome]. Aqui é a [seu nome], da Plenya.
A [quem indicou] comentou que você tinha interesse em conhecer o nosso atendimento.
Que bom ter você por aqui. Posso entender um pouco do seu momento para te orientar melhor?
```

**D) Chegou perguntando preço direto ("quanto custa a consulta?")**
```
Oi, [nome], tudo bem? Aqui é a [seu nome], da Plenya.
Te respondo já o valor, sim. Antes só de uma coisa rápida: a consulta com o Dr. Getúlio é particular
e leva cerca de 60 minutos, porque é uma avaliação completa, não uma consulta de quinze minutos.
O valor é R$ 800,00. Quer que eu já veja uma data para você?
```
*(Para o público que pergunta preço a frio, responder o valor rápido e ancorado é melhor do que enrolar.
Ver objeção 1 se vier o "tá caro".)*

### 3.2 Descoberta (1 a 3 perguntas, nunca um questionário)
```
Me conta um pouco: o que te fez procurar um acompanhamento agora?
```
Conforme a resposta, uma segunda pergunta natural:
```
Entendi. E você já fez exames recentes ou faz acompanhamento com algum médico hoje?
```

> **LGPD e CFM:** não peça histórico clínico detalhado por mensagem. Pergunte o contexto geral
> ("o que te trouxe", "faz acompanhamento hoje"), não diagnósticos, sintomas detalhados ou resultados de
> exame. O lugar disso é a consulta, com o médico.

### 3.3 Valor (conectar a dor ao que a consulta entrega)

Use a resposta da pessoa. Exemplos por situação:

*Cansaço / "faço tudo certo e não vejo resultado":*
```
Esse é exatamente o tipo de situação que a Consulta Plenya foi pensada para olhar.
São cerca de 60 minutos: o Dr. revisa sua história, seus exames e sua rotina, e trabalha com um painel
de exames mais amplo que o de rotina, lido entre o "normal" e o que de fato seria o ideal para você.
No fim, você sai com uma conduta clara, não só com a informação de que "está tudo normal".
```

*"Tive um susto / caso na família":*
```
Faz todo sentido procurar agora. A medicina da Plenya é justamente sobre antecipar, olhar o que ainda
dá para mudar. Na consulta, o Dr. Getúlio faz uma leitura completa do seu histórico e dos seus exames e
monta um plano com você. Quer que eu veja uma data?
```

### 3.4 Plantar o Continuum (de leve, sem fechar)
```
A consulta já resolve muita coisa sozinha. E, dependendo do seu caso, o Dr. pode te apresentar o
Continuum, que é o acompanhamento contínuo com a equipe (médico, nutricionista, psicólogo e educador
físico) ao longo de alguns meses. Mas isso é um passo seguinte, conversado com calma na consulta.
```
*(Plantou. Não fale valor do Continuum. Volte ao agendamento.)*

### 3.5 Agendamento
```
Você prefere presencial, aqui em Londrina, ou online por telemedicina?
```
```
Tenho [dia] às [hora] ou [dia] às [hora]. Algum desses funciona para você?
```
Ao confirmar:
```
Perfeito. Deixei reservado: [dia], [hora], [formato], com o Dr. Getúlio.
A consulta é particular, no valor de R$ 800,00, pago de forma antecipada, no Pix ou cartão.
Para a consulta render bem, traga seus exames mais recentes e a lista de medicamentos que usa.
Vou te enviar a confirmação e, perto da data, um lembrete. Combinado?
```

### 3.6 Pós-agendamento (confirmação e anti-falta)

*Confirmação imediata:*
```
Agendamento confirmado:
[dia], [hora], [presencial: endereço / online: te envio o link no dia].
Qualquer imprevisto, me avise por aqui que a gente remarca.
```
*Lembrete (1 a 2 dias antes):*
```
Oi, [nome]. Passando para confirmar sua consulta [dia] às [hora] com o Dr. Getúlio.
Está tudo certo para você? Não esqueça dos exames anteriores.
```

---

## 4. Roteiro Telefone (falas)

Mesma espinha, em voz. Fale devagar, deixe a pessoa terminar, não atropele. As falas abaixo são base;
soe natural, não decorado.

### 4.1 Atendimento receptivo (a pessoa ligou)
> "Plenya, bom dia. Aqui é a [seu nome], em que posso ajudar?"

Depois de ouvir o motivo:
> "Entendi. Para eu te orientar certo, posso te fazer só duas perguntinhas rápidas?"

### 4.2 Ligação ativa (retorno de formulário/aplicação)
> "Olá, falo com [nome]? Aqui é a [seu nome], da Plenya. Você preencheu nosso formulário no site, e estou
> retornando o seu contato. É um bom momento para a gente conversar dois minutinhos?"

Se for um bom momento, segue. Se não:
> "Sem problema. Qual horário costuma ser melhor para você, de manhã ou à tarde? Te ligo de volta."
*(Marque o retorno com dia e hora. Não deixe em aberto.)*

### 4.3 Descoberta
> "O que te fez procurar um acompanhamento agora?"
>
> *(escuta)*
>
> "E hoje você faz acompanhamento com algum médico, fez exames recentes?"

### 4.4 Valor (exemplo)
> "Pelo que você me conta, é exatamente o tipo de situação que a consulta com o Dr. Getúlio foi pensada
> para olhar. É uma avaliação completa, de cerca de 60 minutos. Ele revisa sua história, seus exames, sua
> rotina, e trabalha com um painel mais amplo que o de rotina. No fim, você sai com uma conduta clara, e não
> apenas com um 'está tudo normal'."

### 4.5 Plantar Continuum
> "A consulta já resolve bastante coisa. E, dependendo do seu caso, o próprio Dr. pode te apresentar o
> Continuum, que é o acompanhamento contínuo com a equipe ao longo de alguns meses. Mas isso é um passo
> seguinte, conversado lá na consulta com calma."

### 4.6 Agendamento
> "Você prefere presencial, aqui na nossa clínica em Londrina, ou online por telemedicina?"
>
> "Eu tenho [dia] às [hora] ou [dia] às [hora]. Qual fica melhor?"
>
> *(ao confirmar)* "Fechado. Vou deixar reservado para você: [dia], às [hora], [formato]. A consulta é
> particular, no valor de R$ 800,00, pago de forma antecipada. No dia, traga seus exames mais recentes e a lista de remédios que você
> usa. Pode me confirmar seu nome completo e um WhatsApp para eu te enviar a confirmação?"

### 4.7 Encerramento da ligação
> "Combinado, [nome]. Está agendado. Vou te mandar a confirmação no WhatsApp agora e, perto da data, um
> lembrete. Foi um prazer falar com você. Até [dia]."

---

## 5. Banco de objeções

Estrutura de cada item: o que a pessoa diz, o princípio para responder, e a resposta nos dois canais.
Regra geral: **acolha primeiro, responda com clareza, devolva o próximo passo.** Nunca discuta, nunca
soe na defensiva.

### Objeção 1: "Tá caro" / "Por que tão caro?"
**Princípio:** não defenda o preço, mostre o que está dentro dele. Ancore na consulta longa e na conduta.
O valor é fixo; não ofereça desconto nem negocie.
- **WhatsApp:** "Entendo. O valor reflete uma consulta completa, de cerca de 60 minutos, com leitura de um
  painel de exames mais amplo e uma conduta construída para o seu caso, não uma consulta rápida de rotina.
  É um cuidado pensado para durar. Quer que eu veja uma data para você conhecer o Dr. e avaliar se faz
  sentido?"
- **Telefone:** "Faz sentido pesar. Só para você ter clareza do que está incluso: é uma avaliação de cerca
  de 60 minutos, com um painel de exames ampliado e uma conduta feita para você. É diferente de uma consulta
  comum. Posso te reservar um horário para você conhecer e decidir com calma?"

### Objeção 2: "Tem desconto?" / "Dá para parcelar?" / "Dá para negociar?"
**Princípio:** firmeza gentil. O valor é único e não negociável, e isso se diz com tranquilidade, sem
constrangimento. Não invente condição que não existe.
- **WhatsApp:** "O valor da consulta é único, R$ 800,00, e não temos negociação de preço. O pagamento é à
  vista, no Pix ou no cartão, feito antes da consulta. Quer que eu veja uma data para você?"
- **Telefone:** "A consulta tem um valor único, de R$ 800,00, e a gente não trabalha com desconto nem
  negociação, viu? O pagamento é à vista, no Pix ou no cartão, antes da consulta. Posso reservar um horário
  para você?"

### Objeção 3: "Vocês não atendem convênio?"
**Princípio:** afirme com naturalidade, sem pedir desculpa, e ofereça o benefício fiscal.
- **WhatsApp:** "Não, o atendimento é exclusivamente particular. Uma informação útil: despesas médicas são
  dedutíveis no Imposto de Renda como gasto com saúde. Quer que eu veja uma data para você?"
- **Telefone:** "Atendemos só no particular, sim. Vale lembrar que a consulta é dedutível no Imposto de
  Renda, como toda despesa médica. Posso ver um horário para você?"

### Objeção 4: "Prefiro presencial" (ou o contrário, "queria que fosse online")
**Princípio:** a Plenya faz os dois com a mesma profundidade. Acolha a preferência.
- **WhatsApp:** "Os dois funcionam, e você escolhe. Presencial aqui em Londrina ou online por
  telemedicina, com a mesma profundidade clínica. Qual fica melhor para você?"
- **Telefone:** "Pode ficar tranquilo, atendemos das duas formas, presencial em Londrina e online, com a
  mesma qualidade. Qual você prefere?"

### Objeção 5: "Já tenho meu médico" / "Já faço check-up"
**Princípio:** a Plenya não substitui, articula. Posicione contra cuidado fragmentado, nunca contra o
outro médico.
- **WhatsApp:** "Que ótimo, e a ideia não é substituir ninguém. O Dr. Getúlio atua como um médico que
  conduz seu cuidado no tempo e conversa com os outros profissionais que já cuidam de você. Muita gente
  chega com exames 'normais' e mesmo assim sentindo que algo não fecha. A consulta olha exatamente esse
  intervalo. Quer experimentar uma avaliação?"
- **Telefone:** "Que bom que você já se cuida. O nosso papel não é substituir o seu médico, é integrar.
  O Dr. olha o conjunto, conduz no longo prazo e conversa com quem já te acompanha. E olha além do 'está
  normal', para o que ainda dá para melhorar. Faz sentido você conhecer?"

### Objeção 6: "Meus exames deram normais, eu estou bem"
**Princípio:** o conceito da janela silenciosa, sem assustar. Antecipar, não amedrontar.
- **WhatsApp:** "Que bom que estão normais. E é justamente aí que entra o nosso olhar: normal não é a mesma
  coisa que ótimo. Existe um intervalo, às vezes de anos, entre um exame 'normal' e algo que muda a rota, e
  é nesse intervalo que dá para agir. A consulta serve para olhar esse espaço com calma. Quer conhecer?"
- **Telefone:** "Fico feliz que estejam bons. O ponto é que 'normal' e 'ótimo' são coisas diferentes. A
  medicina que o Dr. pratica olha esse intervalo entre os dois, que é onde ainda dá para mudar a trajetória.
  Vale como uma avaliação preventiva. Posso ver uma data?"

### Objeção 7: "Preciso pensar" / "Vou falar com meu marido/esposa"
**Princípio:** respeite, não pressione, mas segure um próximo passo concreto.
- **WhatsApp:** "Claro, é uma decisão sua e faz sentido conversar em casa. Posso fazer uma coisa: deixo um
  horário pré-reservado para [dia] e te retorno [dia/hora combinada] para confirmar. Se não for o momento,
  é só me avisar, sem problema. Pode ser?"
- **Telefone:** "Imagina, decisão de saúde a gente toma com calma mesmo. Que tal o seguinte: eu seguro uma
  data provisória e te ligo [dia], aí você já conversou e me diz. Combinamos assim?"

### Objeção 8: "Qual a diferença de uma consulta comum?"
**Princípio:** quatro diferenças concretas, sem floreio.
- **WhatsApp:** "São quatro coisas, na prática: a consulta dura cerca de 60 minutos, não quinze; o Dr.
  trabalha com um painel de exames mais amplo que o de rotina; quando o caso pede, existe uma equipe
  (nutrição, psicologia, educação física) que conversa entre si sobre o seu caso; e o seu progresso é
  acompanhado por uma medida própria, o Escore Plenya, que mostra a evolução no tempo. É um cuidado
  contínuo, não uma consulta avulsa."
- **Telefone:** "Basicamente, a consulta é longa, cerca de 60 minutos; o Dr. olha um painel de exames mais
  completo; tem uma equipe que atua junto quando o caso pede; e o seu progresso é medido ao longo do tempo
  pelo nosso Escore. É outro tipo de acompanhamento."

### Objeção 9: "Esse Continuum é muito tempo, muito compromisso"
**Princípio:** separe consulta de programa. Tire a pressão.
- **WhatsApp:** "Entendo, e fica tranquilo: o Continuum é um passo bem mais à frente, e só faz sentido para
  quem quer construir saúde ao longo de meses. O que estamos falando agora é só a consulta, que já resolve
  bastante por si só. Sem nenhum compromisso além dela. Quer marcar?"
- **Telefone:** "Pode ficar à vontade. O Continuum é outra etapa, para quem decide acompanhar a longo
  prazo. Por ora é só a consulta, que já tem valor sozinha. Sem compromisso nenhum além disso."

### Objeção 10: "Vou pesquisar e depois eu vejo"
**Princípio:** não force; deixe a porta aberta com um próximo passo leve.
- **WhatsApp:** "Perfeito, pesquise com calma. Vou deixar meu contato salvo aqui e, se quiser, te mando o
  link do nosso site para você conhecer o método e o Dr. Getúlio. Quando quiser marcar, é só me chamar
  por aqui. Posso te enviar o link?"
- **Telefone:** "Claro, faça com calma. Posso te mandar por WhatsApp o nosso site, com a explicação do
  método e do Dr. Quando você quiser, me chama que eu já vejo a agenda. Pode ser?"

### Objeção 11: "Estou sem tempo agora"
**Princípio:** ofereça a telemedicina e horários flexíveis; agende mesmo que à frente.
- **WhatsApp:** "Entendo a correria. Por isso a consulta online ajuda bastante, você faz de onde estiver,
  sem deslocamento. E posso ver um horário mais para frente que caiba melhor na sua semana. Quer que eu
  procure?"
- **Telefone:** "Imagino. A gente tem o formato online justamente para isso, sem você precisar se deslocar.
  E consigo um horário um pouco mais à frente, se ajudar. Quer que eu veja?"

### Objeção 12: "Não sou de Londrina"
**Princípio:** a telemedicina resolve. Trate como não-problema.
- **WhatsApp:** "Sem problema nenhum. A consulta online tem a mesma profundidade da presencial, e atendemos
  pacientes de todo o Brasil por telemedicina. Você faz de onde estiver. Quer que eu veja uma data?"
- **Telefone:** "Tranquilo, atendemos por telemedicina pacientes do Brasil inteiro, com a mesma qualidade da
  presencial. Você participa de onde estiver. Posso reservar um horário?"

### Objeção 13: "Isso funciona mesmo? Cadê a prova?"
**Princípio:** credibilidade do médico e do método, sem prometer resultado (CFM).
- **WhatsApp:** "Pergunta justa. O Dr. Getúlio é médico nefrologista com vinte anos de prática, coordena a
  residência de Nefrologia da Santa Casa de Londrina e é autor do livro 'Antes'. O cuidado segue um método
  estruturado, o AGIR, e é medido pelo Escore Plenya ao longo do tempo. Não prometemos milagre; o que
  oferecemos é um acompanhamento sério e mensurável. A consulta é a melhor forma de você sentir isso na
  prática. Quer marcar?"
- **Telefone:** "Entendo a dúvida. O Dr. Getúlio é nefrologista com vinte anos de prática, coordena a
  residência de Nefrologia da Santa Casa e escreveu um livro sobre prevenção. O acompanhamento tem método e
  é medido no tempo. A gente não promete milagre, oferece cuidado sério. A consulta é o melhor jeito de você
  conhecer. Posso ver uma data?"

---

## 6. Fechamento e agendamento

**Sinais de que é hora de fechar:** a pessoa pergunta sobre horários, valor, como funciona o pagamento, ou
diz "acho que faz sentido". Quando ouvir isso, vá para o agendamento, não continue vendendo.

**Dados a coletar (mínimo necessário):**
- Nome completo
- WhatsApp / telefone
- Preferência: presencial ou online
- Melhor dia e horário
- E-mail (para envio de confirmação e, se online, o link)

**Sempre confirme em voz alta / por escrito o combinado:** dia, hora, formato, valor e o que trazer
(exames + medicamentos). A confirmação evita 80% dos mal-entendidos.

**Pagamento:** informe com naturalidade que a consulta custa R$ 800,00, valor único e não negociável, pago
de forma antecipada (à vista, no Pix ou cartão) antes do início. O pagamento confirma o horário reservado.

---

## 7. Anti-falta (no-show) e follow-up

**Cadência de confirmação:**
- No ato: mensagem de confirmação com dia, hora, formato e endereço (ou aviso de que o link virá no dia).
- 1 a 2 dias antes: lembrete pedindo confirmação ("está tudo certo para você?").
- No dia (online): enviar o link com antecedência e um "te espero às [hora]".

**Se a pessoa não confirmar o lembrete:** ligue. Uma ligação curta e gentil recupera muito agendamento que
o silêncio perderia.

**Reativação de lead frio (sumiu durante a conversa):** uma única mensagem, leve, sem cobrança.
```
Oi, [nome]. Passando para saber se você ainda tem interesse em conhecer a Plenya.
Se quiser, deixo um horário reservado para você. Se não for o momento, sem problema, é só me dizer.
```
Se não responder, registre e não insista. Pressão queima o contato para o futuro.

---

## 8. Qualificação: os 6 perfis (e para quem não é)

Use para reconhecer o momento da pessoa e devolver com as palavras dela.

**Para quem a Plenya é (6 situações típicas):**
1. **Faz tudo certo e não vê resposta.** Treina, come bem, dorme, mas a energia não volta e os marcadores
   não melhoram.
2. **Recebeu um susto.** Um exame preocupante, um diagnóstico na família, um aniversário que pesou.
3. **Carrega mais do que a si mesmo.** Sócios, equipe, filhos, pais dependem dela inteira.
4. **Está numa transição que ninguém nomeia.** Sono, humor, corpo mudando, e a resposta que ouve é "ainda
   não é nada" ou "é a idade".
5. **Tem profissionais isolados.** Personal, nutricionista, terapeuta, médico, cada um no seu canto, sem
   ninguém integrando.
6. **Quer estar inteira para quem ainda vai chegar.** Filho pequeno, projetos de longo prazo, gente
   contando com ela.

**Para quem a Plenya não é (diga com tranquilidade):**
- Quem busca resolver só um ponto pontual e urgente. Uma dúvida específica ou um exame para interpretar se
  resolve numa consulta avulsa; o acompanhamento contínuo não é o caminho.
- Quem não quer mudar nada na rotina. O método propõe e acompanha, mas quem executa é a pessoa.
- Quem precisa de pronto-atendimento ou urgência. A Plenya não substitui emergência nem plantão.

> Reconhecer que não é o momento de alguém não é perder um cliente. É preservar a agenda e a confiança.

---

## 9. Cartão de referência rápida

> Para deixar ao lado do telefone.

**Clínica:** Plenya · (43) 99974-8899 · Av. Ayrton Senna da Silva, 500, Torre Pietra, sala 1402, Gleba
Palhano, Londrina/PR · Seg a sex, 8h às 18h · plenyasaude.com.br · @plenyaSaude

**Consulta:** particular, R$ 800,00 (à vista/Pix/cartão, antecipado, não negociável) · cerca de 60 min · presencial (Londrina) ou online · sem convênio
(dedutível no IR) · trazer exames + lista de medicamentos.

**3 diferenças em uma linha:** consulta longa (cerca de 60 min), painel de exames ampliado lido entre o normal e
o ótimo, e acompanhamento medido no tempo pelo Escore Plenya, com equipe integrada quando o caso pede.

**Top 5 objeções (resposta de bolso):**
1. *Tá caro* → "É uma avaliação completa de cerca de 60 min com conduta para o seu caso, não uma consulta
   rápida. Quer conhecer e decidir com calma?"
2. *Não tem convênio?* → "É particular, e é dedutível no IR. Vejo uma data?"
3. *Já tenho médico* → "Não substituímos, integramos. O Dr. conduz e conversa com quem já te acompanha."
4. *Meus exames deram normais* → "Normal não é o mesmo que ótimo. A consulta olha esse intervalo."
5. *Preciso pensar* → "Claro. Deixo um horário pré-reservado e te retorno [dia] para confirmar?"

**CTA padrão:** "Quer que eu veja uma data para você?"

---

## 10. Guardrails (o que nunca fazer)

- **Não diagnosticar nem prometer.** A recepção não dá orientação clínica, não interpreta exame, não promete
  cura nem resultado. Isso é do médico, na consulta (CFM).
- **Não falar valor do Continuum.** A recepção só informa o valor da Consulta. O Continuum é "sob consulta",
  apresentado pelo médico.
- **Não usar o termo "medicina preditiva".** A linguagem da casa é prevenção, antecipação, longevidade.
- **Não citar marcas, produtos ou lojas** (wearables, suplementos, varejistas). Fale por categoria, nunca
  por marca.
- **Não pedir dados clínicos sensíveis por mensagem** (sintomas detalhados, resultados de exame,
  diagnósticos). Pergunte só o contexto geral. O histórico é levantado na consulta (LGPD).
- **Não posicionar contra "médico mais barato".** A comparação certa é com cuidado fragmentado, nunca com
  preço de concorrente.
- **Não pressionar.** Sem urgência forçada, sem insistência depois do não. O público da Plenya recua quando
  sente pressão.
- **Tom escrito e falado:** frases inteiras e conectadas. Sem travessão, sem gírias, sem fechos de efeito,
  sem excesso de emoji ou exclamação.

---

## Fontes (para manutenção)

Conteúdo derivado das fontes canônicas da Plenya. Ao atualizar, conferir contra:
- `apps/site/messages/pt.json` (seções `consultas`, `continuum`, `escore`, perfis "para quem é/não é").
- `packages/brand/src/brand.ts` (tagline, claim, pilares AGIR, público, tom de voz).
- `apps/site-getulio/messages/pt.json` e `docs/seo/01-NAP-master.md` (bio, credenciais, NAP).
- `docs/continuum/plano-aquisicao-marketing.md` (funil, consulta como porta de entrada).
- `scripts/deck-builder/continuum/EDITORIAL.md` (arco comercial, regras de voz, "para quem não é").

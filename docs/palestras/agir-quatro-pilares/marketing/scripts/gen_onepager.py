#!/usr/bin/env python3
"""AGIR one-pager — A4 portrait, hybrid pipeline (gpt-image-2 + PIL composite).

Mirrors gen_onepager_v3.py from the ANTES talk, adapted to the AGIR keynote:
- Hero word is AGIR (not ANTES) — same Didone treatment.
- Cinematic top band: a quiet interior at dawn with FOUR slender vertical beams
  of warm light cutting across a deep petrol floor (visual metaphor for the
  four pillars — A·G·I·R).
- Three takeaways focused on the operational method (4 pilares + Regra dos
  Dois) instead of biomarker count + AGIR mention.
- Bottom claim: VIVA BEM. VIVA MAIS. (same Plenya claim — coherence).

Run:
  python3 gen_onepager.py             # full pipeline
  python3 gen_onepager.py --compose   # skip API, recomposite from existing frame
"""
import json
import base64
import pathlib
...
- Typography: 100% editorial. Tall refined SERIF (Didone/Playfair/Bodoni feel) for the word "AGIR" and headline numerals; modern SANS-SERIF (Söhne/Inter/GT America feel) for body and tags.
...
ASSET: A4 PORTRAIT ONE-PAGER for a premium medical keynote (the AGIR keynote — companion to the ANTES keynote), editorial bookstore quality.
...
1) TOP BAND — first 38% of canvas height (~778 px tall), background petrol #063b4f.
   The CINEMATIC PHOTOGRAPHIC SCENE behind the title — visual metaphor for the four pillars:
   - A quiet interior at dawn, deep petrol shadows.
   - A tall window with FOUR slender vertical mullions on the left third of the frame, casting FOUR PARALLEL BEAMS of warm golden sunset light slanting across the petrol floor.
   - Each beam slightly different in intensity, the four together suggesting structure without literal labels.
   - The rest of the room recedes into petrol darkness with subtle warm highlights at the floor edges.
   - Heavy atmospheric grain, painterly, like a still from a Villeneuve or Sorrentino film.
   - NO PEOPLE. NO furniture. NO objects. NO numbers, letters, or labels visible anywhere in the scene. Just walls, floor, window, and the four slanting bands of light.
   ...
   - Center-anchored, the HERO HEADLINE:
     · "AGIR" — towering editorial serif (Didone/Bodoni), in warm gold #b38645, centered horizontally, occupying about 35-40% of the band height. This is the visual king. Spell exactly A-G-I-R.
     · Beneath: "Os Quatro Pilares" in cream bold sans-serif, refined.
     · Beneath that: "da Longevidade Prática" in cream regular sans-serif, lighter.
...
3) CREAM BODY — remaining 62% of canvas, background cream #eae7da:
   ...
   a) HOOK — petrol bold sans-serif, large, left-aligned with ~12% outer margins:
      "Você já sabe o que tem que fazer — por que não está fazendo?"

   b) SYNOPSIS — petrol regular sans-serif, 4 short lines, justified, comfortable leading:
      "Um framework operacional de quatro pilares — Atividade física, Gestão clínica, Integração mente-corpo, Ritmo circadiano. Quatro casos clínicos reais. Um protocolo de hábito que funciona em três meses, sem sobrecarga e sem fórmula mágica."
   ...
   c) THREE TAKEAWAYS — stacked vertically, each with:
      ...
      Content (verbatim, with proper accents):
        01 · O método
             Quatro letras — A·G·I·R — que organizam tudo.
        02 · A Regra dos Dois
             Dois focos por trimestre. Alto impacto, baixo esforço.
        03 · A virada
             Saber não é fazer. Decidir dois é começar.
   ...
   e) AUTHOR BAND — two columns:
      ...
      RIGHT COLUMN (~70%): bio in petrol sans-serif, left-aligned:
         "Nefrologista com 20 anos de prática clínica. Coordena a Residência de Nefrologia da Santa Casa de Londrina. Fundador da Plenya — clínica de medicina integrada em saúde, performance e longevidade. Autor de ANTES (2026, ISBN 978-65-02-06742-0)."
...
def generate_frame() -> None:
    print("=== Generating AGIR one-pager frame (1152x2048) ===", flush=True)
...
def composite() -> None:
    print("=== Compositing real assets onto AGIR frame ===", flush=True)

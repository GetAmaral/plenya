...
SELECT
    si.id,
    si.name,
    COUNT(sia.article_id) as articles_count,
    CASE
        WHEN COUNT(sia.article_id) >= 2 THEN '✓ OK'
        ELSE '✗ INSUFFICIENT'
    END as status
FROM score_items si
...
SELECT
    ...
    sa.doi
FROM score_items si
JOIN score_item_articles sia ON si.id = sia.score_item_id
JOIN scientific_articles sa ON sia.article_id = sa.id
WHERE si.id = '22952700-63e3-422d-b4b2-179c0785a20e'
